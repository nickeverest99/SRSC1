...
class hjBox {
	...
	IvParameterSpec ivSpec = new IvParameterSpec(config.getIv());
        Key secretKey = new SecretKeySpec(config.getKey(), config.getAlgorithm());
        Cipher cipher = Cipher.getInstance(config.getCipherSuite());
 	...
 	//meu
	  ...
	  buffer = cipher.doFinal(buffer);      // descifrar para por no vlc
